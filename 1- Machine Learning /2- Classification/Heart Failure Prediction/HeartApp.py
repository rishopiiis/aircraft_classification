...
# Load Model
model_RandomForestClassifier = joblib.load('RandomForestClassifier_model.sav')
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# Build Streamlit
st.cache_data.clear()
...
with st.container(height=1400):
    ...
    with col2:
         st.write("### Enter Your Resting Blood Pressure🩸")
         RestingBP = st.slider("Select [mm Hg]:",40,400,120)

         st.write("### Enter Your Oldpeak (ST Depression Induced by Exercise)🏃‍♂️")
         Oldpeak = st.slider("Select:",0,10,0)
         
         st.write("### Enter Your Maximum Heart Rate Achieved⏱️")
         MaxHR = st.slider("Select [bpm]:",50,250,120)

         st.write("### Select Your Exercise Induced Angina")
         ExerciseAngina = st.selectbox("Select:",ExerciseAngina_list_selection)

         st.write("### Select Your Slope of the Peak Exercise ST segment")
...
def predict_heart_failure(df):
    prediction = model_RandomForestClassifier.predict(df)
    prediction_prob = model_RandomForestClassifier.predict_proba(df)  
